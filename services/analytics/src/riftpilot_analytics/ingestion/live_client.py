...
class LiveClientClient:
    ...
    def __init__(
        self,
        base_url: str = "https://127.0.0.1:2999/liveclientdata",
        timeout: float = 1.5,
    ) -> None:
...
def _normalize_score(raw: dict[str, Any] | None) -> Score:
    ...
    return Score(
        ...
        creep_score=int(raw.get("creepScore", 0)),
        ward_score=float(raw.get("wardScore", 0.0)),
    )
...
def _normalize_items(raw_items: list[dict[str, Any]] | None) -> list[ItemState]:
    return [
        ItemState(
            ...
            name=str(item.get("displayName") or item.get("rawDisplayName") or "Unknown item"),
...
def _normalize_stats(raw: dict[str, Any] | None) -> ChampionStats | None:
    ...
    return ChampionStats(
        current_health=float(raw.get("currentHealth", 0.0)),
        max_health=float(raw.get("maxHealth", 0.0)),
        armor=float(raw.get("armor", 0.0)),
        magic_resist=float(raw.get("magicResist", 0.0)),
        attack_damage=float(raw.get("attackDamage", 0.0)),
        ability_power=float(raw.get("abilityPower", 0.0)),
        move_speed=float(raw.get("moveSpeed", 0.0)),
        ability_haste=float(raw.get("abilityHaste", 0.0)),
    )


def _normalize_event(raw: dict[str, Any], aliases: dict[str, str]) -> GameEvent:
    reserved = {"EventID", "EventName", "EventTime", "KillerName", "VictimName"}
    actor_raw = raw.get("KillerName")
    victim_raw = raw.get("VictimName")
    actor = aliases.get(str(actor_raw), str(actor_raw)) if actor_raw is not None else None
    victim = aliases.get(str(victim_raw), str(victim_raw)) if victim_raw is not None else None
    return GameEvent(
        event_id=raw.get("EventID"),
        event_name=str(raw.get("EventName", "UnknownEvent")),
        event_time=float(raw.get("EventTime", 0.0)),
        actor=actor,
        victim=victim,
...
def normalize_all_game_data(payload: dict[str, Any]) -> GameSnapshot:
    ...
    active_id = str(
        active_raw.get("riotId")
        or active_raw.get("summonerName")
        or payload.get("activePlayerName")
        or "Unknown player"
    )
    ...
    for raw in payload.get("allPlayers") or []:
        ...
        for alias_key in ("riotId", "summonerName", "riotIdGameName"):
            alias = raw.get(alias_key)
            if alias:
                aliases[str(alias)] = riot_id
        team_raw = str(raw.get("team", "UNKNOWN")).upper()
        team = Team(team_raw) if team_raw in Team._value2member_map_ else Team.UNKNOWN
        players.append(
            PlayerState(
                riot_id=riot_id,
                champion_name=str(raw.get("championName", "Unknown")),
                team=team,
                level=int(raw.get("level", 1)),
                is_active=riot_id == active_id,
                score=_normalize_score(raw.get("scores")),
                items=_normalize_items(raw.get("items")),
                stats=active_stats if riot_id == active_id else None,
                current_gold=(
                    float(active_gold)
                    if riot_id == active_id and active_gold is not None
                    else None
                ),
            )
        )
    ...
    if not players:
        players = [
            PlayerState(
                riot_id=active_id,
                champion_name=str(active_raw.get("championName", "Unknown")),
                is_active=True,
                stats=active_stats,
                current_gold=float(active_gold) if active_gold is not None else None,
            )
        ]
    ...
    return GameSnapshot(
        source=SnapshotSource.LIVE_CLIENT,
        game_time=float(game_data.get("gameTime", 0.0)),
        game_mode=str(game_data.get("gameMode", "UNKNOWN")),
        map_name=str(game_data.get("mapName", "UNKNOWN")),
        active_player_id=active_id,
        players=players,
        events=[_normalize_event(event, aliases) for event in events_raw],
    )

...
def test_arbitrary_snapshot_analysis_and_simulation(tmp_path) -> None:
    ...
    simulation = client.post("/v1/simulate/defense", json={"health": 900, "armor": 50, "magic_resist": 40, "incoming_physical": 700, "incoming_magic": 500, "incoming_true": 100, "add_health": 250, "add_armor": 30, "add_magic_resist": 20})
...
def test_history_missing_returns_404(tmp_path) -> None:
    client = make_client(tmp_path)
    assert client.get("/v1/history/not-found").status_code == 404
